...
def spherical_projection_40962(mesh_data, hemisphere='lh'):
    ...
    Args:
        mesh_data (tuple): (vertices, triangles) of the input mesh
        hemisphere (str): 'lh' or 'rh' for left or right hemisphere
    
    Returns:
        tuple: (sphere_coords, sphere_triangles, target_coords, target_triangles)
               All outputs have exactly 40962 vertices
    """
    ...
    cmd = f"python utils/mesh/S3MAP-main/s3all.py -i {abs_path} --save_interim_results True --device CPU"
    os.system(cmd)
    
    # Check output files
    if not os.path.exists(sphere_vtk) or not os.path.exists(target_vtk):
        raise FileNotFoundError("S3MAP failed to create output files")
    ...
    # Verify vertex count
    if len(target_coords) != 40962 or len(sphere_coords) != 40962:
        raise ValueError("Expected 40962 vertices for both meshes")
